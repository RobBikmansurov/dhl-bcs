  ...
  module Bcs
    module V2
      class ExportDocument
        ...
        PROPERTIES = %i[invoice_number export_type export_type_description terms_of_trade place_of_commital additional_fee
        ...
        def self.build(export_doc_positions = [], **attributes)
          ...
          export_doc_positions.each do |export_doc_position|
            if export_doc_position.is_a?(Hash)
              array_of_export_doc_positions << ExportDocPosition.build(export_doc_position)
            end
          end
        ...
        def export_type=(export_type)
          unless EXPORT_TYPES.include?(export_type)
            raise Dhl::Bcs::Error,
                  "No Valid export_type #{export_type}, Please use one of these: #{EXPORT_TYPES.join(',')}"
          end
        ...
        def terms_of_trade=(terms_of_trade)
          unless TERMS_OF_TRADES.include?(terms_of_trade)
            raise Dhl::Bcs::Error,
                  "No Valid terms_of_trade #{terms_of_trade}, Please use one of these: #{TERMS_OF_TRADES.join(',')}"
          end
        ...
        def to_soap_hash
          ...
          if (export_type == 'OTHER') ^ export_type_description
            raise Dhl::Bcs::Error,
                  'export_type_desription must be set, as export_type is set to OTHER.'
          end
          ...
          h['ExportDocPosition'] = export_doc_positions.map(&:to_soap_hash)
          h
        end
      end

      class ExportDocPosition
        PROPERTIES = %i[description country_code_origin customs_tariff_number amount net_weight_in_kg
                        customs_value].freeze
        attr_accessor(*PROPERTIES)
